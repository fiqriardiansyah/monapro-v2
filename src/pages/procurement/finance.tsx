...
const FinancePage = <T extends TDataFinance>() => {
    ...
    const getList = useQuery([query ? financeService.search : financeService.getAll, page, query], async () => {
        if (query) {
            const req = await financeService.Search({ page: page as any, query: query as any, type: PROCUREMENT_TYPE });
            return req.data.data;
        }
        const req = await financeService.GetAll({ page, type: PROCUREMENT_TYPE });
        return req.data.data;
    });
